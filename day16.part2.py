...
Cost: TypeAlias = int
_COSTS_CACHE: dict[str, dict[ValveName, Cost]] = {}
...
def get_costs_to_get(
    ...
    if current_name in _COSTS_CACHE:
        return _COSTS_CACHE[current_name]
    ...
    _COSTS_CACHE[current_name] = valve_name_to_cost
    return _COSTS_CACHE[current_name]
...
def _cache_key(
    ...
    minutes_left_el: int,
    current_name: str,
    current_elephant_name: str,
    opened: set[str],
) -> str:
    key1 = sorted([current_name, current_elephant_name])
    key2 = sorted([minutes_left, minutes_left_el])
    key3 = "-".join(sorted(list(opened)))
...
def get_total_given(
    ...
    cache_key = _cache_key(
        minutes_left, minutes_left_el, current_name, current_elephant_name, opened
    )
    ...
    # ELEPHANT
    costs_to_get_el = get_costs_to_get(valves, current_elephant_name)
    costs_to_get_valves_el = (
        (valves[name], cost_to_get)
        for (name, cost_to_get) in costs_to_get_el.items()
        if name not in opened
    )
    effect_per_valves_el = tuple(
        (v.name, ((minutes_left_el - cost - 1) * v.flow_rate))
        for v, cost in costs_to_get_valves_el
    )

    if len(effect_per_valves) == 0:
        assert len(effect_per_valves_el) == 0
        return 0

    if len(effect_per_valves_el) == 0:
    ...
    # if cache_key in CACHE:
    CACHE[cache_key] = max(different_paths)
    return CACHE[cache_key]
...
def main():
    ...
    total = get_total_given(
        ...
        minutes_left_el=minutes_left,
        current_name="AA",
        current_elephant_name="AA",
        opened={"AA"},
    )

    # 2504 too low (part 2 final input)
    print(total)
    print(f"{cache_hits=}")
    return
