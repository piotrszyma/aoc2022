...
def main():
    ...
    all_pos = {elf.pos for elf in elfs}
    min_row = min(r for (r, c) in all_pos)
    max_row = max(r for (r, c) in all_pos)
    min_col = min(c for (r, c) in all_pos)
    max_col = max(c for (r, c) in all_pos)

    row_size = abs(max_row - min_row) + 1
    col_size = abs(max_col - min_col) + 1
    total_size = row_size * col_size
    empty_ctr = total_size - len(elfs)
